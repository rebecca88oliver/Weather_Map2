    ...
    function searchCity (cityName) {
        ...
        if (cityName !== '') {
            ...
            $.ajax({
                ...
                $("#item1").empty().append("<p class='title3'>" + result.name + " (" + todayDate + ")</p>");
        ...
        $.ajax({
            ...
            for (var i = 0; i < res.list.length; i++) {
                ...
                if (res.list[i].dt_txt.includes("09:00:00")){
                    ...
                    newCard.addClass("card_5 forecast  align-items-end nes-container");
                    newCard.append("<h3>" + displayDate + "</h3>");

                    if (res.list[i].main.temp <= 58) {
                        newCard.append("<i>").addClass("bi bi-emoji-dizzy");
